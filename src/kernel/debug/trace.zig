...
//! Designed to scale to SMP without refactoring.
//!
//! This module uses IRQ disable only, no locks. Safe to call
//! from scheduler with scheduler lock held. Must never acquire locks here.
...
var active_cpus: u32 = 0;
var enabled: bool = false;
var enabled_mask: u64 = 0xffff_ffff_ffff_ffff;
...
pub inline fn emit(id: EventId, a: usize, b: usize, c: usize) void {
    ...
    if ((enabled_mask & (@as(u64, 1) << bit)) == 0) return;
    const cpu = currentCpu();
    const ring = &rings[cpu];
    const idx = ring.head & RING_MASK;
    ring.head +%= 1;
    ring.events[idx] = .{
